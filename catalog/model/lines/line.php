<?php
class ModelLinesLine extends Model {	
	...
	public function add_comment($data=array()){
		...
		$query = $this->db->query("insert into aa_lines_comments (user_id,line_id,title,content,email,create_date) values (
					'".$data['user_id']."',
					'".$data['line_id']."',
					'".$data['comment_title']."',
	...
	public function getComments($data){
		$query = $this->db->query("SELECT * from aa_lines_comments where line_id='".$data['line_id']."' order by create_date desc");
		$rows=$query->rows;
